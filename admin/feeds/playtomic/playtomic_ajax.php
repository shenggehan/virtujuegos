...
$query = mysql_query("SELECT * FROM ava_playtomic WHERE $where AND visible = 0 ORDER BY id DESC LIMIT $from, $max_results");
...
echo '
<div id="game-'.$go['id'].'" class="'.$class.'"><div class="manage_column0"><img src="'.$go['thumb_url'].'" width="40" height="40"></div><div id="tgame_name'.$go['id'].'" class="mochi_column">'.$go['name'].' '.$highscore_image.' <span class="mochi_details">&nbsp;&nbsp;Category: '.$go['category'].'</span><br /><div class="mochi_description">'.$go['description'].'</div></div>
...
$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM ava_playtomic WHERE $where AND visible = '0'"),0);
