...
$feed = 'http://feedmonger.mochimedia.com/feeds/query/?format=json&limit='.$feed_setting['max_feed'].$feed_setting['feed_params'];
...
$count = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM ava_mochi WHERE gametag='$game[game_tag]'"),0);
if ($count != 1) {
...
$sql = mysql_query("INSERT INTO ava_mochi (gametag, name, description, thumb_url, file_url, width, height, author, author_link, category, instructions, featured, tags, highscores) VALUES ('$game[game_tag]', '$name', '$description', '$thumb_url', '$swf_url', '$game[width]', '$game[height]', '$author', '$a_link', '$category', '$instructions', '$featured', '$tag_list', '$highscores')") or die (mysql_error());

$i = $i + 1;
}}
echo $i.' games added to database<br /><br />';
?>
<div class="mochi_buttons"><div class="mochi_button"><a href="?task=mochi#page=1&cat=All">Mochi Feed</a></div><div class="mochi_button"><a href="https://www.mochimedia.com/pub/catalog">Mochi Catalog</a></div><div class="mochi_button"><a href="?task=feed_settings">Mochi Settings</a></div></div>
<br style="clear:both" />