...
$query = mysql_query("SELECT * FROM ava_mochi WHERE $where AND visible = 0 ORDER BY id DESC LIMIT $from, $max_results");
...
if ($go['highscores'] == 1) {
	$highscore_image = '<img src="'.$setting['site_url'].'/images/trophy.png" title="Highscore enabled" />';
}
else {
	$highscore_image = '';
}
...
echo '
<div id="game-'.$go['id'].'" class="'.$class.'"><div class="manage_column0"><img src="'.$go['thumb_url'].'" width="40" height="40"></div><div id="tgame_name'.$go['id'].'" class="mochi_column">'.$go['name'].' '.$highscore_image.' <span class="mochi_details">&nbsp;&nbsp;Category: '.$go['category'].' &nbsp;By: <a href="'.$go['author_link'].'" class="manage_link">'.$go['author'].'</a></span><br /><div class="mochi_description">'.$go['description'].'</div></div>

<div class="mochi_column3" id="play-icon-'.$go['id'].'"><img src="images/go.png" width="24" height="24" onclick="PlayGame('.$go['id'].', '.$height.');" /></div>

<div class="mochi_column3" id="download-icon-'.$go['id'].'"><img src="images/dl.png" width="24" height="24" onclick="AddGame('.$go['id'].', '."'".$go['gametag']."'".');" /></img></div>
...
$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM ava_mochi WHERE $where AND visible = 0"),0);
