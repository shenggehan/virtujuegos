...
$query = mysql_query("SELECT * FROM ava_spil WHERE $where AND visible = 0 ORDER BY id DESC LIMIT $from, $max_results");
...
echo '
<div id="game-'.$go['id'].'" class="mochi_item"><div class="manage_column0"><img src="'.$go['thumb_url'].'" width="40" height="40"></div><div id="tgame_name'.$go['id'].'" class="mochi_column">'.$go['name'].' <span class="mochi_details">&nbsp;&nbsp;Category: '.$go['category'].'</span><br /><div class="mochi_description">'.$go['description'].'</div></div>

<div class="mochi_column3" id="download-icon-'.$go['id'].'"><img src="images/dl.png" width="24" height="24" onclick="AddGame('.$go['id'].', '."'".$go['spil_id']."'".');" /></img></div>
...
$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM ava_spil WHERE $where AND visible = '0'"),0);
