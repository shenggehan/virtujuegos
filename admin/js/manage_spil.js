...
function LoadGames() {
	...
	var url = "feeds/spil/spil_ajax.php?page=" + hashvalues['page'] + '&cat=' + hashvalues['cat'];
...
function AddGame(id, spil_id) {
	AjaxPost("feeds/spil/get_spil_game.php", "spil_id=" + spil_id, 
			 function () {
...
function PlayGame(id, height) {
	...
	AjaxPost("feeds/spil/play_spil_game.php", "id=" + id, 
			 function () {
			 	if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
			 		$('game-' + id).style.height = height+'px';
...
function RejectGame(id) {
		doMove(id, 50, 1);
		AjaxPost("feeds/spil/reject_spil_game.php", "id=" + id, 
			 function () {}
	)
}
