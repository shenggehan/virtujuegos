...
function LoadGames() {
	...
	var url = "feeds/playtomic/playtomic_ajax.php?page=" + hashvalues['page'] + '&cat=' + hashvalues['cat'];
...
function AddGame(id, gametag) {
	...
	AjaxPost("feeds/playtomic/get_playtomic_game.php", "gametag=" + gametag, 
			 function () {
			 	if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
...
function PlayGame(id, height) {
	$('play-icon-' + id).innerHTML = '<img src="images/load.gif">';
	AjaxPost("feeds/playtomic/play_playtomic_game.php", "id=" + id, 
			 function () {
			 	if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
			 		$('game-' + id).style.height = height+'px';
...
function RejectGame(id) {
		doMove(id, 50, 1);
		AjaxPost("feeds/playtomic/reject_playtomic_game.php", "id=" + id, 
			 function () {}
	)
}
