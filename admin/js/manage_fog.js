...
function clickrecall(thisfield) {
	if (thisfield.value == "") {
		$('search_box').value = 'Search '+ $('mochi_category')[$('mochi_category').selectedIndex].innerHTML;
...
function goTo(page, scroll) {
	...
	location.hash = 'page='+page+'&cat='+$('mochi_category').value+search;
	if (scroll == 1) {
...
function LoadGames() {
	...
	var url = "feeds/fog/fog_ajax.php?page=" + hashvalues['page'] + '&cat=' + hashvalues['cat'];

	$('mochi_category').value = hashvalues['cat'];
...
function AddGame(id, fog_id) {
	AjaxPost("feeds/fog/get_fog_game.php", "fog_id=" + fog_id + "&category=" + $('category'+id).value, 
			 function () {
			 	$('reject-icon-' + id).innerHTML = '';
				$('play-icon-' + id).innerHTML = '';
				$('download-icon-' + id).innerHTML = '<img src="images/load.gif">';
			 	$('category-container-' + id).innerHTML = '';
			 	
			 	if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
					$('game-' + id).className = 'mochi_added';
					var p = eval("(" + xmlHttp.responseText + ")");
					$('category-container-' + id).innerHTML = '<a href="../index.php?task=view&id='+p.new_id+'"><img src="images/go.png" width="24" height="24"></a>';
...
function PlayGame(id, height) {
	...
	AjaxPost("feeds/fog/play_fog_game.php", "id=" + id, 
			 function () {
			 	if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
			 		$('game-' + id).style.height = height+'px';
...
function RejectGame(id) {
		doMove(id, 50, 1);
		AjaxPost("feeds/fog/reject_fog_game.php", "id=" + id, 
			 function () {}
	)
}
