...
function LoadGames() {
	...
	var url = "feeds/mochi/mochi_ajax.php?page=" + hashvalues['page'] + '&cat=' + hashvalues['cat'];
...
function AddGame(id, gametag) {
	...
	AjaxPost("feeds/mochi/get_mochi_game.php", "gametag=" + gametag, 
			 function () {
			 	if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
...
function PlayGame(id, height) {
	$('play-icon-' + id).innerHTML = '<img src="images/load.gif">';
	AjaxPost("includes/play_mochi_game.php", "id=" + id, 
			 function () {
			 	if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
			 		$('game-' + id).style.height = height+'px';
...
function RejectGame(id) {
		doMove(id, 50, 1);
		AjaxPost("includes/reject_mochi_game.php", "id=" + id, 
			 function () {}
	)
}
